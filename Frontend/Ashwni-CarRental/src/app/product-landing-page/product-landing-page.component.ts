...
export class ProductLandingPageComponent implements OnInit{
  ...
  constructor(private  getProduct : CarrentalService , private router: Router){
...
availableCar(id : any){
  console.log("my car id req",id);
  this.getProduct.isavailable(id).subscribe((result: any) => {
     if(result == true)
     {
      this.router.navigate([`viewdetails/${id}`]);
     }
     else {
       alert("This Car is already rented try to rent other car");
     }
  })
}
  ...
  sortProducts(sortdata : any){
    this.collection.forEach((object : any) => {
      if (sortdata == object.car_Category)
      {
